...
p8:
	...
	cat lib/header.txt label.txt build/build.lua lib/gfx_header.txt build/gfx.txt lib/map_header.txt build/map.txt build/audio.txt > build/$(p8_filename)
	rm build/map.txt
	rm build/gfx.txt
	rm build/audio.txt
